...
class Sarsa_lambda():
    ...
    def __init__(self, nteta=3000, gamma=1, lmbda=0.2, alpha=0.5, epsilon=0.0):
    ...
    def learn(self, n_episodes, maxstep):
        ...
        for e in range(n_episodes):
            ...
            for stp in range(maxstep):#loop over steps in same episode
                ...
                if new_state==None:
                    self.teta+= (self.alpha/self.ntiles)*delta*self.z # (self.alpha/self.ntiles)*delta *self.z
                    break
                self.compute_features(state) #compute features for new  state
                self.compute_action_value(None) #compute new states values for action value
                delta += self.gamma * self.Q[action]  #discount factor
